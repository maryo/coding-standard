...
* `fixable` (default: `true`): allows the reference to be fixed automatically. Set it to `false` to only report the reference — `Throwable` also covers `Error`, so the replacement changes which types are handled.
